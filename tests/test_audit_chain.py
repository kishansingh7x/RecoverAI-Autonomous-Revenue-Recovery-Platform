...
def test_ptp_tracker_maintains_hash_chain(audit_db):
    """Verifies that follow-up actions and stops generated by ptp_tracker compute valid hashes."""
    ...
    cursor.execute("""
        INSERT INTO transactions (
            transaction_id, customer_id, customer_name, amount, payment_method,
            status, channel, created_at, customer_opted_out
        ) VALUES ('TXN_PTP_01', 'CUST_PTP', 'Sunita Rao', 5000.0, 'upi', 'failed', 'web', '2026-09-01 10:00:00', 0)
    """)
    ts = "2026-09-01 10:01:00"
    h1 = compute_event_hash(ts, "TXN_PTP_01", "send_reminder_sms", "Initial reminder", 1, GENESIS_HASH)
    cursor.execute("""
        INSERT INTO actions_log (
            action_id, transaction_id, action_type, reasoning, attempt_number,
            timestamp, previous_hash, event_hash
        ) VALUES ('act_ptp_init', 'TXN_PTP_01', 'send_reminder_sms', 'Initial reminder', 1, ?, ?, ?)
    """, (ts, GENESIS_HASH, h1))
    conn.commit()
    conn.close()

    as_of = datetime.strptime("2026-09-15 12:00:00", "%Y-%m-%d %H:%M:%S")
    process_promises_to_pay(db_path=audit_db, seed=42, as_of=as_of, use_llm=False)
